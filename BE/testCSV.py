...
def run_identification_test() -> None:
    ...
    This version is configured to run against all images found in the S3 bucket
    under the defined prefix.
    """
    
    # 1. Dynamically load ALL S3 keys for batch testing
    cos = initialize_cos_client()
    if not cos:
        print("Cannot proceed without a valid COS connection.")
        return

    # --- CHANGE MADE HERE: Using dynamic list_s3_keys instead of hardcoded key ---
    S3_KEYS_TO_TEST = list_s3_keys(cos, COS_BUCKET_NAME, COS_PREFIX) 
    # -----------------------------------------------------------------------------
    
    if not S3_KEYS_TO_TEST:
        print("🛑 No image files found to test. Exiting.")
        return

    print(f"\n--- Starting Full Batch Test ({len(S3_KEYS_TO_TEST)} images) ---")
        ...
        for s3_key in S3_KEYS_TO_TEST:
            ...
            try:
                ...
                if search_results:
                    ...
                    top_candidate_name = top_candidate.get('fish_name', 'N/A')
                    ...
                    match_found = any(
                        cand.get('fish_name', '').lower().replace('-', ' ') == expected_clean 
                    ...
                    all_candidates_str = "; ".join([
                        f"R{i+1}: {cand.get('fish_name', 'N/A')} ({cand.get('score', 0.0):.4f})"
    ...
    print(f"\n--- Full Batch Test Complete! Results saved to {OUTPUT_CSV} ---")
